...
From a given $E_d(0^-)$ measured or modeled right under the ice sheet.
...
df %>%
  ...
  labs(
    title = str_wrap("Snow and sea ice thickness during the 2016 ice camp campaigns", 50),
...
The C-OPS has an exceptional signal-to-noise ratio (SNR).
...
df %>% 
  ...
  geom_path(size = 1, linejoin = "bevel") +
  scale_color_manual(values = color) +
  guides(color = guide_legend(
    keywidth = 0.15,
    keyheight = 0.15,
...
SimulO was used to simulate the propagation of light under a sea-ice surface (100 m $\times$ 100 m) containing **a 5-meters radius melt pond**.
