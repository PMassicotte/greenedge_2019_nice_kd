...
- $K_d(z)z~[m^{-1}]$ is the vertical diffuse attenuation coefficient describing the rate at which light decreases with increasing depth.  
...
In open water, downward irradiance follows quite well a monotonically exponential decrease with depth.
...
In ice-covered water, subsurface light maxima at a depth of around 10 m are clearly visible between 400 and 560 nm.
...
<b><i>Propose a new method to estimate average irradiance over large spatially heterogeneous area.</i></b>
...
Less influenced by the surface spatial heterogeneity (**no subsurface maximum**).
...
p1 + 
  ...
  plot_annotation(
    ...
    subtitle = "Profiles have been normalized to their maximum. <i style='color:#E23636'>At 10 m depth, the effects of surface heterogeneity<br>is reduced.</i>",
    theme = theme(plot.title = element_text(margin = margin(2,0,2,0)), plot.subtitle = element_markdown(lineheight = 1.1))
  )
...
- Show that $K_{Lu}$ can be used as a proxy to estimate the mean $\overline{K_{d}}$ in the water column using:
  
  - *in-situ* C-OPS measurements
  
  - Monte-Carlo simulations (SimulO)
...
- At an ice camp where the water depth was 360 m, from 20 April to 27 July 2016.

- Snow cover and landfast ice thickness varied between 0–49 cm and 106–149 cm, respectively.
...
<center><img src="img/cops_open_water.png" height="200"/></center>
...
reference_profile %>%
  ...
  labs(
    ...
    subtitle = "The <span style='color:#E23636'>**thick red line**</span> represents the average <i>E</i><sub>*d*</sub> simulated for the <span style='color:#E23636'>**entire area**</span>. The <span style='color:#4B8D1F'>**green lines**</span> show the propagated <i>E</i><sub>*d*</sub><br> at different distances from the melt pond."
  ) +
  scale_x_continuous(
